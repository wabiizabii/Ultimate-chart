...
# Helper functions (extract_data_from_report_content_sec6, save_..._sec6) are assumed to be defined globally.
...
with st.expander("📂  Ultimate Chart Dashboard Import & Processing", expanded=True):
    ...
    # Ensure uploader_key_version is initialized (should be in PART 1.4)
    if 'uploader_key_version' not in st.session_state:
        st.session_state.uploader_key_version = 0

    # Use a unique key for the file uploader, incorporating the version
    uploaded_file_statement = st.file_uploader(
        "ลากและวางไฟล์ Statement Report (CSV) ที่นี่ หรือคลิกเพื่อเลือกไฟล์",
        type=["csv"],
        key=f"ultimate_stmt_uploader_main_sec6_{st.session_state.uploader_key_version}"
    )
    ...
    st.checkbox("⚙️ เปิดโหมด Debug (แสดงข้อมูลที่แยกได้ + Log การทำงานบางส่วนใน Console)", 
                ...
                key="debug_statement_processing_v2_checkbox") # Ensure key is distinct if used elsewhere
    # Update session state if checkbox changes
    st.session_state.debug_statement_processing_v2 = st.session_state.debug_statement_processing_v2_checkbox
    ...
    if uploaded_file_statement is not None:
        ...
        try:
            ...
            print(f"Warning: Could not compute MD5 hash for file '{file_name_stmt}': {e_hash_stmt}")
        ...
        if not active_portfolio_id_for_stmt_import:
            ...
            # No st.stop() here, let the script finish this run, uploader will reset on next interaction.
        else:
            st.info(f"ไฟล์ที่อัปโหลด: {file_name_stmt} (ขนาด: {file_size_stmt} bytes, Hash: {file_hash_stmt})")
            
            gc_stmt = get_gspread_client()
            if not gc_stmt:
                st.error("ไม่สามารถเชื่อมต่อ Google Sheets Client ได้. กรุณาตรวจสอบการตั้งค่า Secrets.")
            else:
                ws_stmt_dict = {}
                # Worksheet definitions (ensure these constants are defined in PART 1.3)
                    ...
                    for ws_name, specs in worksheet_definitions_stmt.items():
                        try:
                            ...
                            if ws_stmt_dict[ws_name].row_count > 0:
                                ...
                                except Exception: pass # Ignore if cannot get headers, will try to update
                            ...
                            try:
                                new_ws_stmt = sh_log_stmt.add_worksheet(title=ws_name, rows=specs.get("rows", "1000"), cols=specs.get("cols", "50")) # Ensure enough cols
                            ...
                            sheets_ok_stmt = False; break
                    if not sheets_ok_stmt:
                        st.stop() # Stop if sheets setup failed
                except Exception as e_setup_stmt: # Covers API errors for gc.open or other general setup
                    st.error(f"❌ เกิดข้อผิดพลาดในการเข้าถึง Spreadsheet '{GOOGLE_SHEET_NAME}': {e_setup_stmt}")
                    st.stop()
                
                # Check if all required worksheets are now in ws_stmt_dict
                for ws_name_key in worksheet_definitions_stmt.keys():
                    if ws_name_key not in ws_stmt_dict or ws_stmt_dict[ws_name_key] is None:
                        st.error(f"❌ ไม่สามารถเข้าถึงหรือสร้าง Worksheet '{ws_name_key}' ได้อย่างสมบูรณ์. การประมวลผลหยุดลง.")
                        st.stop()
                ...
                if previously_processed_successfully:
                    ...
                    # No st.rerun() here, let the user decide next action. Uploader key not incremented.
                else:
                    ...
                    try:
                        ...
                        # If initial log fails, we might not want to proceed with further processing in this run.
                    ...
                    if initial_log_ok_stmt:
                        ...
                        final_status_stmt = "Failed_Unknown" # Default status if not explicitly set
                        processing_notes_stmt = []
                        equity_updated_this_run = False # Flag to track if equity was updated in this specific run
                        ...
                        try:
                            ...
                            if st.session_state.get("debug_statement_processing_v2", False):
                                ...
                                st.json({k: (v.to_dict(orient='records') if isinstance(v, pd.DataFrame) else v) for k, v in extracted_stmt_data.items()}, expanded=False)
                                st.write("--- END DEBUG ---")
                            ...
                            if not extraction_successful:
                                st.warning("ไม่สามารถแยกข้อมูลที่มีความหมายจากไฟล์ได้ หรือไฟล์ไม่มีข้อมูล Transactional/Summary ที่เพียงพอ")
                            ...
                            if not processing_errors_stmt:
                                ...
                                elif summary_note_stmt != "no_data_to_save" and not summary_ok_stmt : # Check if save failed
                                     st.error(f"❌ ({WORKSHEET_STATEMENT_SUMMARIES}) Summary: ล้มเหลว ({summary_note_stmt})"); processing_errors_stmt = True
                            ...
                            # Determine final_status_stmt after all processing attempts
                            if not processing_errors_stmt:
                                final_status_stmt = "Success"
                            else:
                                # If already set by a specific exception, keep that, otherwise mark as PartialSave
                                if final_status_stmt == "Failed_Unknown": # Only override if it's still the default unknown
                            ...
                            processing_errors_stmt = True 
                            if st.session_state.get("debug_statement_processing_v2", False): st.exception(e_main_proc_stmt)
                        ...
                        try:
                            if WORKSHEET_UPLOAD_HISTORY in ws_stmt_dict and ws_stmt_dict[WORKSHEET_UPLOAD_HISTORY] is not None:
                                ...
                                print(f"CRITICAL Error: Worksheet object for '{WORKSHEET_UPLOAD_HISTORY}' is None. Cannot update final status.")
                                if 'final_status_stmt' not in locals() or final_status_stmt == "Success": final_status_stmt = "Failed_UpdateHistory_Access" # Mark a specific error
                            ...
                            if 'final_status_stmt' not in locals() or final_status_stmt == "Success": final_status_stmt = "Failed_UpdateHistory_Exception"
                            if 'processing_notes_stmt' in locals() and isinstance(processing_notes_stmt, list): processing_notes_stmt.append(f"ErrorUpdatingUploadHistoryCritical: {e_update_hist_final_stmt}")

                        # 2. Increment uploader key version
                        ...
                        if final_status_stmt == "Success":
                            ...
                            if equity_updated_this_run: # Use the flag set during equity update
                            ...
                            st.balloons()
                        # ... (other elif conditions for different final_status_stmt to display errors) ...
                        elif final_status_stmt == "Failed_UnicodeDecode":
                            detail_note = processing_notes_stmt[-1] if processing_notes_stmt else 'Unknown Decode Error Detail'
                            st.error(f"การประมวลผลไฟล์ '{file_name_stmt}' ล้มเหลว: เกิดข้อผิดพลาดในการ Decode ไฟล์. กรุณาตรวจสอบ Encoding (ควรเป็น UTF-8). รายละเอียด: {detail_note}")
                        elif final_status_stmt.startswith("Failed_MainProcessing_"):
                            detail_note = processing_notes_stmt[-1] if processing_notes_stmt else 'Unknown Main Processing Error Detail'
                            st.error(f"การประมวลผลไฟล์ '{file_name_stmt}' ล้มเหลว: เกิดข้อผิดพลาดระหว่างประมวลผลหลัก. รายละเอียด: {detail_note}")
                        ...
                        elif final_status_stmt.startswith("Failed_UpdateHistory"): # Covers Access and Exception
                             st.error(f"การประมวลผลไฟล์ '{file_name_stmt}' (Batch ID '{import_batch_id_stmt}') สำเร็จบางส่วน แต่มีปัญหาสำคัญในการอัปเดตประวัติการอัปโหลด (UploadHistory). โปรดตรวจสอบ Log และ Google Sheets.")
                        else: # Failed_Unknown or other specific statuses
                    ...
                    # This 'else' corresponds to 'if initial_log_ok_stmt:'
                    else: 
                        st.error("มีปัญหาในการเริ่มต้นบันทึกประวัติการอัปโหลดไฟล์ กรุณาลองอีกครั้ง")
                        st.session_state.uploader_key_version += 1 
                        st.rerun()
                # This is the end of the 'if not previously_processed_successfully:' block
            # This is the end of the 'else:' for 'if not active_portfolio_id_for_stmt_import:'
        # This is the end of the 'if uploaded_file_statement is not None:' block

    st.markdown("---") # End of SEC 6 expander
