...
with st.expander("📂  Ultimate Chart Dashboard Import & Processing", expanded=True):
    ...
    def save_transactional_data_to_gsheets(ws, df_input, unique_id_col, expected_headers, data_type_name, portfolio_id, portfolio_name, source_file_name="N/A", import_batch_id="N/A"):
        ...
        try:
                    ...
                    if all_sheet_records:
                        ...
                        if unique_id_col in df_existing_sheet_data.columns and 'PortfolioID' in df_existing_sheet_data.columns:
                            ...
                            if not df_portfolio_data.empty and unique_id_col in df_portfolio_data.columns:
                                # Ensure IDs from sheet are also stripped of whitespace for accurate comparison
            ...
            if unique_id_col not in df_to_check.columns:
                st.error(f"Unique ID column '{unique_id_col}' is MISSING from the data extracted for '{data_type_name}'. Deduplication might be inaccurate.")
                # Proceeding without deduplication if unique_id_col is missing from input,
                # but this means all data will be treated as new for this part of the logic.
                # Or, decide to return False if this is critical. For now, let it attempt to save all.
                new_df = df_to_check
            else:
                # Ensure unique_id_col values are stripped of whitespace before comparison
            ...
            if st.session_state.get("debug_statement_processing_v2", False) and data_type_name == "Deals":
                ...
                st.dataframe(df_to_check[[unique_id_col]].head(10) if unique_id_col in df_to_check else "Unique ID col not found in df_to_check")
    ...
    # (แก้ไขให้รับ ws และ import_batch_id)
    def save_deals_to_actual_trades(ws, df_deals_input, portfolio_id, portfolio_name, source_file_name="N/A", import_batch_id="N/A"):
        ...
        # *** สำคัญ: ต้องแน่ใจว่า "Deal_ID" (หรือ Unique ID ที่ถูกต้อง) ถูกสร้าง/อ่านมาใน df_deals_input จาก extract_data_from_report_content ***
    ...
    # --- ฟังก์ชัน save_results_summary_to_gsheets (แก้ไขให้มีการตรวจสอบเนื้อหาซ้ำ) ---
    def save_results_summary_to_gsheets(ws, balance_summary_data, results_summary_data, portfolio_id, portfolio_name, source_file_name="N/A", import_batch_id="N/A"):
        try:
            if ws is None:
                ...
                return False # Return only boolean as per original file's expectation
    ...
    uploaded_file_statement = st.file_uploader( 
        ...
        type=["csv"],
        key="ultimate_stmt_uploader_v3_final" # New unique key
    )
    ...
    if uploaded_file_statement:
        ...
        # Define WORKSHEET_UPLOAD_HISTORY if not already defined globally
        ...
        # Check for duplicate file upload in UploadHistory BEFORE logging the current attempt as "Processing"
        ...
        try:
            ...
            st.warning(f"Could not read UploadHistory for duplicate check: {e_hist_read}")
            ...
            if not extracted_sections: 
                ...
                if should_save_summary_now:
                    if balance_summary or results_summary_data_ext:
                        ...
                        # Ensure save_results_summary_to_gsheets is called with ws_dict
                        ...
                        # The function save_results_summary_to_gsheets now returns only True/False
                        # The st.info for skipping is handled inside the function
                        save_results_details['Summary'] = {'ok': summary_ok, 'status': 'processed', 'notes': f"Summary: Processed, Success={summary_ok}"}
                        if summary_ok: 
                            # The function itself will st.info if it skipped, so we don't need to repeat it here
                            # Only show general success if it wasn't skipped and was OK
                            if not (is_duplicate_file_found and not any_new_transactional_data_added): # i.e. if it wasn't skipped by the outer logic
