...
with st.expander("📂  Ultimate Chart Dashboard Import & Processing", expanded=True):
    ...
    if uploaded_file_statement is not None: # Check if a file is uploaded
        ...
        st.session_state.uploader_key_suffix += 1 # <<< เพิ่มค่า Suffix ของ Key เพื่อให้ Uploader รีเซ็ตใน Rerun ถัดไป
        st.rerun() # สั่ง Rerun ทันทีเพื่อให้ Key ใหม่มีผลและ Uploader แสดงผลเป็นค่าว่าง
